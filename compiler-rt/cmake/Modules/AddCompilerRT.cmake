...
function(add_compiler_rt_object_libraries name)
  ...
  foreach(libname ${libnames})
    if(${libname} STREQUAL "clang_rt.profile-x86_64")
      log_info("grep_me1")
    else()
      log_debug("grep_me2")
    endif()
    log_debug("add_library(${libname} OBJECT ${LIB_SOURCES})")
    log_debug("LIB_DEPS: ${LIB_DEPS}")
    log_debug("LIB_CFLAGS: ${LIB_CFLAGS}")
    #
    log_debug("set_target_compile_flags(${libname} ${extra_cflags_${libname}} ${target_flags})")
    log_debug("set_property(TARGET ${libname} APPEND PROPERTY COMPILE_DEFINITIONS ${LIB_DEFS})")
    log_debug("set_target_properties(${libname} PROPERTIES FOLDER \"Compiler-RT Libraries\")")
...
function(add_compiler_rt_runtime name type)
  ...
  foreach(libname ${libnames})
    ...
    if(type STREQUAL "OBJECT")
      ...
      # add_library(clang_rt.profile-x86_64 STATIC GCDAProfiling.c;...
      # set_target_compile_flags(clang_rt.profile-x86_64 -m64;-fno-lto;-fPIC;-Wno-pedantic;-DCOMPILER_RT_HAS_ATOMICS=1;-DCOMPILER_RT_HAS_FCNTL_LCK=1;-DCOMPILER_RT_HAS_UNAME=1)
      # set_target_link_flags(clang_rt.profile-x86_64 )
      # set_property(TARGET clang_rt.profile-x86_64 APPEND PROPERTY COMPILE_DEFINITIONS )
      # set_target_output_directories(clang_rt.profile-x86_64 /home/wsh/qc/llvm-project/tmptmp/./lib/clang/11.0.0/lib/linux)
      # install(TARGETS clang_rt.profile-x86_64 ARCHIVE DESTINATION lib/clang/11.0.0/lib/linux COMPONENT;clang_rt.profile-x86_64 LIBRARY DESTINATION lib/clang/11.0.0/lib/linux COMPONENT;clang_rt.profile-x86_64 RUNTIME DESTINATION lib/clang/11.0.0/lib/linux COMPONENT;clang_rt.profile-x86_64)
      log_debug("add_library(${libname} ${type} ${sources_${libname}})")
      log_debug("set_target_compile_flags(${libname} ${extra_cflags_${libname}})")
      log_debug("set_target_link_flags(${libname} ${extra_link_flags_${libname}})")
      log_debug("set_property(TARGET ${libname} APPEND PROPERTY COMPILE_DEFINITIONS ${LIB_DEFS})")
      log_debug("set_target_output_directories(${libname} ${output_dir_${libname}})")
      log_debug("install(TARGETS ${libname} ARCHIVE DESTINATION ${install_dir_${libname}} ${COMPONENT_OPTION} LIBRARY DESTINATION ${install_dir_${libname}} ${COMPONENT_OPTION} RUNTIME DESTINATION ${install_dir_${libname}} ${COMPONENT_OPTION})")
